#!/usr/bin/env bash
# §3 decision-10: три прогона задачи-02 с включённым LSP.
# Шаги и максимум контекста — посегментно по отметкам лога сервера.
set -euo pipefail
...
for i in $(seq 1 "$RUNS"); do
  ...
  printf 'run%-2s wall=%4ss шагов=%-3s контекст=%-6s lsp=%-3s rc=%s решена=%-3s | %s\n' \
