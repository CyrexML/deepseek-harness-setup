...
/**
 * The capability index: resident minimal tools (read / write / edit / bash /
 * grep) cannot cover these, so the model must search
...
const UNLOCKABLE_INDEX = [
  'web_search / web_fetch — internet search and page retrieval',
  'subagent / subagent_fork — delegate work to a sub-agent with its OWN context window',
  'skill — load a skill\'s full instructions by name',
  'glob — find files by name pattern (grep searches contents, glob searches names)',
  'read_image — read image files',
  'todo_write — task tracking',
  'ask_user_question — ask the user a question',
  'workflow / ralph — multi-agent workflow scripts and iterative loops',
...
export function apply(ctx) {
  ctx.tools.register({
    ...
    description: [
      ...
      '',
      'This session starts with a minimal resident set: read, write, edit, bash, grep. Everything else is unlocked on demand through this tool.',
      '',
