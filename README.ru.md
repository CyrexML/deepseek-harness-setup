...
- С телефона — через Cloudflare Tunnel, включается в `config.json`; подробности в
  [docs/MOBILE.ru.md](docs/MOBILE.ru.md).
...
Сам агент об этом знает: первый запуск обучения на CUDA не выполняется, а
возвращает измерение — сколько видеопамяти свободно, сколько держит модель — и
те же три выхода, поэтому вместо OOM-трассировки через несколько минут вы
получаете объяснение сразу. Дальше плагин не мешает: если вы освободили карту
или хотите попробовать, повторный запуск проходит.
